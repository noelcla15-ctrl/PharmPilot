...
# 2.5/3.0 models are strict; we must explicitly disable filters for medical study content
safety_settings = {
...
# ✅ Correct model names
flash_model = genai.GenerativeModel("gemma-3-4b", safety_settings=safety_settings)
quiz_model = genai.GenerativeModel("gemma-3-27b", safety_settings=safety_settings)
...
def extract_objectives_from_slide_texts(slide_texts, max_slides=8):
    ...
    # FIX: Double braces for f-string safety
    prompt = f"""
...
if nav == "Review":
    ...
    # ✅ NEW: Adjustable Session Limit in Sidebar
    st.sidebar.markdown("### ⚙️ Session Settings")
    session_limit = st.sidebar.number_input(
        "Max Cards per Session", 
        min_value=10, 
        max_value=500, 
        value=50, 
        step=10,
        help="Increase this if you have a big exam coming up!"
    )
    ...
    with db_cursor() as (conn, c):
        # ✅ UPDATED QUERY: Sorts by Exam Date first, then due date
        c.execute("""
            ...
            ORDER BY e.exam_date ASC NULLS LAST, c.next_review ASC 
            LIMIT %s
        """, (today, session_limit))
        
        cards_due = c.fetchall()

        # ✅ NEW: Total Pending Count for "Real Demand"
    ...
    if not st.session_state.session_active:
        if not cards_due:
            ...
            with col1:
                # ✅ NEW: Shows "50 / 120" (Limit / Actual)
                st.metric("Cards Due", f"{total_due} / {total_pending}")
            with col2:
                if next_ex:
